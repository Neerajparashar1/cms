...
try {
    ...
    $email_stmt->execute([':user_id' => $_SESSION['user_id']]);
    ...
    // Get student details from students table using email
    $user_query = "SELECT u.name, u.email, s.id as student_id, s.roll_no, s.college_id, s.course_id, s.session_id, s.batch_id,
    ...
    $user_stmt->execute([':email' => $user['email']]);
    ...
    $track_query = "INSERT INTO admit_card_downloads (admit_card_id, student_id, ip_address) 
                    VALUES (:admit_card_id, :student_id, :ip_address)";
    try {
